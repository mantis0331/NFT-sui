...
import { searchLoading, searchMyListings } from "../../../../redux/state/search";
...
import MyListingsList from "../../../../components/layouts/profile/Lists/MyListingsList";
...
const MyListingsPanel = ({ title }) => {
  ...
  const fetchInfo = async (searchAndSortParams) => {
    await dispatch(searchMyListings(searchAndSortParams))
      .then(() => {
  ...
  const unlistListing = async (item) => {
    const res = await unlistNFT(item);
