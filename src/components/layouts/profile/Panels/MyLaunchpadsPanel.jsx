...
import { searchMyLaunchpads, searchLoading } from "../../../../redux/state/search";
import MyLaunchpadsList from "../../../../components/layouts/profile/Lists/MyLaunchpadsList";
...
const MyLaunchpadsPanel = ({ title }) => {
  ...
  const fetchInfo = async (searchAndSortParams) => {
    await dispatch(searchMyLaunchpads(searchAndSortParams))
      .then(() => {
  ...
  return (
    <div>
      ...
      <PaginationWrapper
        ...
        count={count}
      >
        <MyLaunchpadsList layout={layout} setCount={setCount} setPages={setPages} />
      </PaginationWrapper>
    </div>
  );
};

export default MyLaunchpadsPanel;
