...
import { searchLoading, searchLoanListings } from "../../../../redux/state/search";
...
const RentalsPanel = ({ title }) => {
  ...
  const fetchInfo = async (searchAndSortParams) => {
    await dispatch(searchLoanListings(searchAndSortParams))
      .then(() => {
  ...
  return (
    <div>
      ...
      <RentModal item={modalShow} onHide={() => handleHideModal()} />
    </div>
  );
};
